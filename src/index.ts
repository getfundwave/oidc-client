...
interface IOIDCClient {
  refreshTokenLock: boolean;
  refreshPath: string;
  baseUrl: string;
  BASE_HEADERS: Record<string, string>;
}

interface IOIDCClientOptions extends IOIDCClient {
...
export class OIDCClient implements IOIDCClient {
  ...
  constructor(options: IOIDCClientOptions) {
  ...
  async prepareHeaders(HEADERS: Object) {
  ...
  verifyTokenValidity() {
    ...
    try {
      ...
      return exp?.exp && exp.exp >= (new Date().getTime() + 10000) / 1000;
    } catch (err) {
      return false;
    }
  }
